...
const Pagination = ({ children, currentPage, totalResults, resultsPerPage, onChange, className, ...props }) => {
    ...
    return (
        ...
        <div className={computedClassName}>
            ...
            {firstPage > 1 && (
                <>
                <Button onClick={() => goToPage(1)} type='secondary' active={current === 1}>1</Button>
                {firstPage > 2 && <Button type='secondary' disabled={true} style={{padding:'0',width:'35px'}}>...</Button> }
                </>
            )}
            ...
            {lastPage < totalPages && (
                <>
                {lastPage < totalPages - 1 && <Button type='secondary' disabled={true} style={{padding:'0',width:'35px'}}>...</Button>}
                <Button onClick={() => goToPage(totalPages)}  type='secondary' active={current==totalPages}>{totalPages}</Button>
                </>
            )}
