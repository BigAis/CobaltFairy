...
function DateRangePicker({
	...
	const computedClassName = classNames('date-picker-container', 'date-range-picker', className)
	...
	const getDateFormat = () => {
		...
		return convertFormat(format) + (pickerType === 'datetime' ? ` ${convertFormat(timeFormat)}` : '')
	}
	...
	useEffect(() => {
		if (initialStartDate) {
			...
			if (hasMinDate && newStartDate.getTime() < new Date(minDate).getTime()) {
	...
	}, [initialStartDate, initialEndDate, minDate, hasMinDate])
	...
	return (
		<div className={computedClassName}>
			<DatePicker
				...
				minTime={pickerType !== 'date' && startDate && isSameDay(startDate, minDate) ? minTime : undefined}
				maxTime={pickerType !== 'date' && startDate && isSameDay(startDate, minDate) ? maxTime : undefined}
				customInput={<DateRangeInput pickerType={pickerType} dateRange={formatDateRange()} type={type} active={active} inactive={inactive} disabled={disabled} />}
				{...props}
			/>
		</div>
	)
...
const DateRangeInput = forwardRef(({ value, onClick, pickerType, dateRange, type, active, inactive, disabled }) => {
	...
	return (
		<button onClick={handleClick} className={computedClassName} disabled={disabled} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
			{pickerType === 'time' ? (
...
DateRangePicker.propTypes = {
	...
	type: PropTypes.oneOf(['primary', 'secondary', 'action']),
